...
async function translateText(text, targetLanguage = null) {
  ...
  // Build prompt based on whether target language is specified
  const getPrompt = (textToTranslate) => {
    if (targetLanguage) {
      return `You are a Telegram bot translator. Auto-detect the source language and translate the following text to ${targetLanguage}.
...
${textToTranslate}`;
    ...
    return `You are a Telegram bot translator. Translate the following text to English. If it's already in English, translate to Arabic. 
...
${textToTranslate}`;
  ...
  const prompt = getPrompt(sanitized);
...
async function staticCommands(text, chatId, userId, msg) {
  try {
    ...
    // Handle /translate with optional target language
    const translateMatch = sanitizedText.match(/^\/translate(?:@tagallesisbabot)?\s*(?:to\s+(.+))?$/i);
    if (translateMatch !== null) {
      const targetLanguage = translateMatch[1] ? translateMatch[1].trim() : null;
      
      if (msg.reply_to_message && msg.reply_to_message.text) {
        await bot.sendChatAction(chatId, "typing");
        const translation = await translateText(msg.reply_to_message.text, targetLanguage);
        const langInfo = targetLanguage ? ` to **${targetLanguage}**` : "";
        await bot.sendMessage(chatId, `🌐 **Translation${langInfo}:**\n\n${translation}`, { parse_mode: "Markdown" });
      } else {
        await bot.sendMessage(
          chatId,
          "❓ **How to translate:**\n\n• `/translate` — Reply to a message to translate (English ↔ Arabic toggle)\n• `/translate to french` — Translate to French\n• `/translate to spanish` — Translate to Spanish\n• `/translate to {language}` — Translate to any language 🌍",
          { parse_mode: "Markdown" }
        );
      }
    }
...
export default async function handler(event) {
  try {
    ...
    // Clear reminder by index (supports multiple comma-separated indexes)
    if (
      ...
      if (remindersData.reminders.length === 0) {
        ...
        await bot.sendMessage(
          chatId,
          "❓ **How to clear reminders:**\n\n• `/clearreminder 1` — Clear reminder #1\n• `/clearreminder 1,2,3` — Clear reminders #1, #2, and #3\n• `/clearreminder 5,2,8` — Clear multiple reminders at once",
          { parse_mode: "Markdown" }
        );
      } else {
        // Parse comma-separated indexes
        const indexStrings = indexInput.split(",").map(s => s.trim()).filter(s => s);
        const parsedIndexes = indexStrings.map(s => parseInt(s, 10));
        ...
        for (let i = 0; i < parsedIndexes.length; i++) {
          const idx = parsedIndexes[i];
          if (isNaN(idx)) {
            invalidIndexes.push(indexStrings[i]);
          } else if (idx < 1 || idx > remindersData.reminders.length) {
            outOfRangeIndexes.push(idx);
          } else if (!validIndexes.includes(idx)) {
            validIndexes.push(idx);
          }
        }
        
        if (invalidIndexes.length > 0 && validIndexes.length === 0) {
          await bot.sendMessage(chatId, `❌ **Invalid index format:** ${invalidIndexes.join(", ")}. Please provide valid numbers.`, { parse_mode: "Markdown" });
        } else if (outOfRangeIndexes.length > 0 && validIndexes.length === 0) {
          await bot.sendMessage(chatId, `❌ **Out of range:** ${outOfRangeIndexes.join(", ")}. Valid range is 1-${remindersData.reminders.length}.`, { parse_mode: "Markdown" });
        } else if (validIndexes.length === 0) {
          await bot.sendMessage(chatId, "❌ **No valid indexes provided.** Please check your input.", { parse_mode: "Markdown" });
        } else {
          ...
          let responseMsg = `✅ **Cleared reminder${validIndexes.length > 1 ? "s" : ""}:** ${validIndexes.sort((a, b) => a - b).join(", ")}`;
          
          if (invalidIndexes.length > 0 || outOfRangeIndexes.length > 0) {
            responseMsg += "\n\n⚠️ **Skipped:**";
            if (invalidIndexes.length > 0) {
              responseMsg += `\n• Invalid format: ${invalidIndexes.join(", ")}`;
            }
            if (outOfRangeIndexes.length > 0) {
              responseMsg += `\n• Out of range: ${outOfRangeIndexes.join(", ")}`;
            }
          }
